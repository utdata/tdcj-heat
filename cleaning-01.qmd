...
We've done the grunt work, now we need to pivot our data longer based on the unit. 

```{r}
indoor_units <- daily_indoor_dated |> 
  pivot_wider(
    names_from = unit,
...
```{r}
indoor_units_clean <- indoor_units |> 
  unite(col = "ramsey", ramsey, ramsey_2) |> 
mutate(
    ramsey = str_remove(ramsey, "_NA") 
  ) 
