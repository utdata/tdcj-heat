...
ramsey <- daily_indoor_dated |> 
  mutate(unit = case_when(grepl('ram', unit, ignore.case = TRUE) == TRUE ~ "Ramsey", TRUE ~ unit))

ramsey |> filter(grepl('ram', unit, ignore.case = TRUE))
```

## Adding columns

This code chunk will add new columns

```{r}
indoor_units_clean <- ramsey |> 
  select(unit, temperature, date)
...
```{r}
indoor_units_clean |> write_rds("data-processed/pearson-01-heat-all.rds")
```
